...
Run: `npx tsc --noEmit`
Expected: FAILS, with errors only in `src/components/TourCard.tsx`,
`src/components/TourGallery.tsx`, `src/components/TourHero.tsx`, and
`src/components/TourDetails.tsx` (each renders a `Tour` field as a plain
string/array today; the field is now `Localized<...>`). This is expected —
Tasks 9 and 12 fix these four files. Do not fix any of them in this task.
If `tsc` reports errors in any *other* file, stop and investigate before
continuing.
...
## Task 12: Wire `TourHero.tsx` and `TourDetails.tsx` — all `tsc` errors resolved
...
- Produces: the last two files with compile errors from Task 4 are fixed —
  `npx tsc --noEmit` should be fully clean after this task.
...
task that touches a `Tour`-consuming component, so the full type chain
from Task 4 is now resolved end to end.

- [ ] **Step 4: Commit**

```bash
git add src/components/TourHero.tsx src/components/TourDetails.tsx
git commit -m "Wire TourHero and TourDetails copy to useT() — full build clean"
```
...
- [ ] **Step 5 (controller, live browser check):** Open any
  ...
  reverts to the original English content exactly.
